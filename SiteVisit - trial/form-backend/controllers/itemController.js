...
// Create inventory outward and decrease item quantity (site_id removed)
    ...
    connection.query(itemQuery, [item_id], (itemErr, itemResult) => {
        ...
        const insertQuery = `
            INSERT INTO inventoryoutward (
                ...
                dispatched_by, remark, site_name, scope, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
        `;
        const values = [
            item_id, quantity_displacement, purpose, date, destination,
            dispatched_by, remark || null, site_name, scope
        ];
...
// Get all inventory inward records
